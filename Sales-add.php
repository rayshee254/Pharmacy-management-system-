...
<aside class="sidebar">
        ...
        if (isset($_POST['add'])) {
            ...
            $sql = "INSERT INTO `sales` (`Medicine Name`, `Customer ID`, `Quantity`, `Total price`, `Payment method`, `Date`) 
                    VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("siisss", $medicineName, $customerId, $quantity, $totalPrice, $paymentMethod, $dateOfPurchase);
            $stmt->execute();
