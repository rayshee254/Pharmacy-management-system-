...
$sql = "SELECT `Medicine Name`, `Quantity`, `Category`, `Price`, `Expiry Date` FROM `meds`";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<a class='button1 edit-btn' href='Inventory-update.php?med=" .$row['Medicine Name']. "&qty=" . $row['Quantity'] . "&cat=" . $row['Category'] . "&sp=" . $row['Price'] . "&edate=" . $row['Expiry Date'] . "'>Edit</a>";
        echo "<a class='button1 del-btn' href='Inventory-delete.php?med=" .$row['Medicine Name']. "&qty=" . $row['Quantity'] . "&cat=" . $row['Category'] . "&sp=" . $row['Price'] . "&edate=" . $row['Expiry Date'] . "'><ion-icon name='trash-outline'></ion-icon></a>";
        echo "</td>";
        echo "</tr>";
    }
    echo "</table>";
