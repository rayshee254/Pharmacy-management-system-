...
<aside class="sidebar">
    ...
    <table id="table1">
        ...
        $sql = "SELECT `INVOICE ID`, `INVOICE DATE`, `CUSTOMER ID`, `MEDICINE NAME`, `TOTAL AMOUNT`, `PAYMENT METHOD` FROM `invoices`";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row["INVOICE ID"] . "</td>";
                echo "<td>" . $row["INVOICE DATE"] . "</td>";
                ...
                echo "<a class='button1 print-btn' href='print-invoice.php?id=" . $row['INVOICE ID'] . "'><ion-icon name='print-outline'></ion-icon></a>";
                echo "<a class='button1 del-btn' href='invoice-delete.php?id=" . $row['INVOICE ID'] . "'><ion-icon name='trash-outline'></ion-icon></a>";
